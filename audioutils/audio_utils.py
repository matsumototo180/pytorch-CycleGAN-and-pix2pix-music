...
class AU():
    def __init__(self, opt):
        self.opt = opt
        if not self.opt.output.exists():
            self.opt.output.mkdir()

    def convertMP3ToWAV(self, path):
        if path.suffix != ".mp3":
            raise Exception('Error: input file is not mp3')
        
        out = str(self.opt.output) + "/" + path.stem + ".wav"
        ...
        if not self.opt.stereo:
            cmd = "ffmpeg -i " + str(path) + " -ar " + str(self.opt.sr) + " -ac 1 " + out 
        else:
            cmd = "ffmpeg -i " + str(path) + " -ar " + str(self.opt.sr) + " " + out
